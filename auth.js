...
async function tbAccountOrders(session){const res=await fetch(TB_ACCOUNT_FUNCTION,{headers:{Authorization:`Bearer ${session.access_token}`,apikey:TB_SUPABASE_KEY,'Content-Type':'application/json'},body:undefined});const data=await res.json().catch(()=>({}));if(!res.ok||!data.success)throw new Error(data.error||'Could not load your orders.');return data.orders||[];}
